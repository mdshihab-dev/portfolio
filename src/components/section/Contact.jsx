  ...
  return (
   <>
    <div id='contact' className=' bg-light-sub-main dark:bg-dark-sub-main'>
      <div className=' container px-1.5 py-16 lg:py-24'>
        <Heading title={title} />
        ...
        <div className=' py-6 lg:my-12'>
          ...
          <span className=' left-2.5 lg:left-4 relative flex items-center justify-center'>
            ...
            <MdContentCopy onClick={()=> handleCopy('email')} className={`${isCopyOne ? ' scale-0' : ' scale-100'} absolute cursor-pointer transition-all dark:hover:text-violet-700`}/>
          </span>
          ...
          <div className=' mt-4 flex items-center justify-center text-xl lg:text-3xl text-heading dark:text-white'>
            ...
            <span className=' left-4 relative flex items-center justify-center'>
              <MdContentCopy onClick={()=> handleCopy('number')} className={`${isCopyTwo ? ' scale-0' : ' scale-100'} absolute cursor-pointer transition-all dark:hover:text-violet-700`}/>
          ...
          <div className=' text-center mt-4 '>
            <div className=' flex items-center justify-center  gap-2.5  text-2xl'>
              <Link to={fbProfile} target='_blank' className={styles}>
                <FaFacebookF className='text-heading dark:text-white dark:group-hover:text-violet-700 transition-all'/>
              </Link>
              <Link to={githubProfile} target='_blank' className={styles}>
                <LuGithub className='text-heading dark:text-white dark:group-hover:text-violet-700 transition-all'/>
              </Link>
              <Link to='/' target='_blank' className={styles}>
                <FaLinkedinIn className='text-heading dark:text-white dark:group-hover:text-violet-700 transition-all'/>
              </Link>
          </div>
          </div>
        </div>
